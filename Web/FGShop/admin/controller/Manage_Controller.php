...
class Manage_Controller extends Base_Controller
{
    ...
    */
    public function index()
    {
        ...
        $list_product = $this->model->Product->getTable($pages, $token, "", "", $filter);
        $num_rows = $this->model->Product->getNumRow("", "", $filter);
        ...
        if ($filter == 1) {
            $table_name = "Product is out of stock ($num_rows)";
        } else if ($filter == 2) {
            $table_name = "Product is coming out of stock ($num_rows)";
        } else if ($filter == 3) {
            $table_name = "Product is in stock ($num_rows)";
        } else {
            $table_name = "Product is out of stock ($num_rows)";
        }
